...
def field_prepare_variables(step):
    ...
    global_inhibition = tf.Variable(step.global_inhibition, name=step.name + ".global_inhibition", trainable=True, constraint=lambda x: tf.clip_by_value(x, -1.0, 0.0))
...
def field_compute_time_invariant_variable_variant_tensors(shape, interaction_kernel_positional_grid, resting_level,
                                                          interaction_kernel_weight_pattern_config):
    resting_level_tensor = tf.multiply(tf.ones(shape), resting_level)
...
@tf.function
def field_time_step(time_step_duration, shape, bin_size, resting_level,
                    time_scale, sigmoid_beta, global_inhibition, noise_strength,
                    interaction_kernel_weight_pattern_config,
                    interaction_kernel_positional_grid,
                    resting_level_tensor,
                    interaction_kernel, input=None, activation=None):
    ...
    :param Tensro resting_level: resting level of the field
    ...
    resting_level_tensor = tf.multiply(tf.ones(tuple([int(x) for x in shape])), resting_level)
    interaction_kernel_weight_pattern_tensor = compute_weight_pattern_tensor(interaction_kernel_weight_pattern_config,
                                                                              interaction_kernel_positional_grid)
    ...
    conv_result = convolve(output, interaction_kernel_weight_pattern_tensor) * bin_size
